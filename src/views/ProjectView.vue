<template>
  <div class="xl:flex w-screen h-screen" :class="{ 'text-white dark': !light }">
    <div
      class="w-screen xl:w-1/2 h-1/3 xl:h-screen flex"
      :class="{ 'bg-black': !light, 'bg-gray-200': light }"
    >
      <router-link :to="'/' + light">
        <svg
          fill="currentColor"
          viewBox="0 0 20 20"
          class="absolute ml-4 mt-4 hidden lg:block w-16"
          :class="{ 'text-black': light, 'text-white': !light }"
        >
          <path
            fill-rule="evenodd"
      ...
      <div class="mx-auto my-auto">
        <div
          ...
          class="bg-gray-300 mx-auto mt-8 flex text-xl hover:text-6xl rounded-lg w-64 h-48 xl:w-96 xl:h-64"
        >
          <span class="my-auto mx-auto xl:text-black">
            Media Demo Comming Soon
          </span>
        </div>
        <img
          v-if="info.Media.type === 'img'"
          :src="'/images/' + info.Media.link"
    ...
    <div class="w-screen xl:w-1/2 flex" :class="{ dark: !light }">
      <div class="mx-auto my-auto w-3/4">
        ...
        <p class="text-xl xl:text-2xl mt-8 text-justify">
          {{ info.description.full }}
        </p>
        <div class="flex mt-8">
          <p class="hidden xl:block text-xl font-medium content-center py-2">
            Technologies Used:
          </p>
        ...
        <div class="pb-32 xl:pb-0">
          <div v-for="links in info.link" :key="links" class="w-full">
            <a :href="links.link" target="_blank" rel="noopener noreferrer">
              <div
                class="p-2 lg:p-4 mt-4 rounded-lg text-xl transition duration-200 ease-in-out border-2 text-center"
                :class="{
                  'bg-black text-white hover:bg-white hover:text-black border-black': light,
                  'bg-white text-black hover:bg-transparent hover:text-white border-white': !light,
                }"
              >
                {{ links.text }}
              </div>
...
export default {
  ...
  created() {
    ...
    axios
      .get("/projects.json")
      .then((response) => (this.info = response.data.projects[this.id]));
  },
};
</script>
